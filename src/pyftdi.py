...
__all__ = ['pyftdi', 'pyftdiError']


class pyftdiError(IOError):
...
class pyftdi(object):
    ...
    def __init__(self):
        ...
        self.in_ep = None
        self.out_ep = None
        self.bitbang_mode = pyftdi.BITMODE_RESET
        self.latency = 0
    ...
    def open(self, vendor=0x403, product=0x6001, interface=0, serialNumber=None):
        """Open a new interface to the specified FTDI device"""
        
        deviceList = self._get_devices(vendor, product)
        
        self.usb_dev = None
        if serialNumber <> None:
            #find the device with the right serialNumber
            
            for dev in deviceList:
                deviceSerialNumber = usb.util.get_string(dev, 255, dev.iSerialNumber)
                if deviceSerialNumber == serialNumber:
                    self.usb_dev = dev
                    break;
            if self.usb_dev == None:
                raise pyftdiError('Unable to find device with the serial number: %s' % serialNumber)
        else:
            #grab the first device in the list
            self.usb_dev = deviceList[0]        
    ...
    def bitbang_enabled(self):
        ...
        return not (self.bitbang_mode == pyftdi.BITMODE_RESET)

    def set_baudrate(self, baudrate):
        """Change the current interface baudrate"""
        if self.bitbang_enabled:
            baudrate *= pyftdi.BITBANG_CLOCK_MULTIPLIER
        actual, value, index = self._convert_baudrate(baudrate)
        delta = 100*abs(float(actual-baudrate))/baudrate
        if delta > pyftdi.BAUDRATE_TOLERANCE:
            raise AssertionError('Cannot represent baudrate')
        try:
            if self.usb_dev.ctrl_transfer(pyftdi.REQ_OUT,
                                          pyftdi.SIO_SET_BAUDRATE, value,
                                          index, '', self.usb_write_timeout):
                raise pyftdiError('Unable to set baudrate')
            self.baudrate = baudrate
        except usb.core.USBError, e:
            raise pyftdiError('UsbError: %s' % str(e))

    def purge_rx_buffer(self):
        """Clear the read buffer on the chip and the internal read buffer."""
        if self._ctrl_transfer_out(pyftdi.SIO_RESET, SIO_RESET_PURGE_RX):
            raise pyftdiError('Unable to set baudrate')
        # Invalidate data in the readbuffer
        pyftdi.readbuffer_offset = 0
        pyftdi.readbuffer = array.array('B')

    def purge_tx_buffer(self):
        """Clear the write buffer on the chip."""
        if self._ctrl_transfer_out(pyftdi.SIO_RESET, SIO_RESET_PURGE_TX):
            raise pyftdiError('Unable to set baudrate')
    ...
    def set_bitmode(self, bitmask, mode):
        ...
        if self._ctrl_transfer_out(pyftdi.SIO_SET_BITMODE, value):
            raise pyftdiError('Unable to set bitmode')
        self.bitbang_mode = mode

    def read_pins(self):
        """Directly read pin state, circumventing the read buffer.
           Useful for bitbang mode."""
        pins = self._ctrl_transfer_in(pyftdi.SIO_READ_PINS, 1)
        if not pins:
            raise pyftdiError('Unable to read pins')
        return pins[0]
    ...
    def set_latency_timer(self, latency):
        """Set latency timer
           ...
           load on the usb bus."""
        if not (pyftdi.LATENCY_MIN <= latency <= pyftdi.LATENCY_MAX):
            raise AssertionError("Latency out of range")
        if self._ctrl_transfer_out(pyftdi.SIO_SET_LATENCY_TIMER, latency):
            raise pyftdiError('Unable to latency timer')

    def get_latency_timer(self):
        """Get latency timer"""
        latency = self._ctrl_transfer_in(pyftdi.SIO_GET_LATENCY_TIMER, 1)
        if not latency:
            raise pyftdiError('Unable to get latency')
        return latency[0]
    ...
    def poll_modem_status(self):
        ...
        """
        value = self._ctrl_transfer_in(pyftdi.SIO_POLL_MODEM_STATUS, 2)
        if not value or len(value) != 2:
            raise pyftdiError('Unable to get modem status')
    ...
    def modem_status(self):
        ...
        for pos, byte_ in enumerate(value):
            for b,v in enumerate(pyftdi.MODEM_STATUS[pos]):
                if byte_ & (1<<b):
                    status.append(pyftdi.MODEM_STATUS[pos][b])
        return tuple(status)
    ...
    def set_flowctrl(self, flowctrl):
        ...
        try:
            if self.usb_dev.ctrl_transfer(pyftdi.REQ_OUT,
                                          pyftdi.SIO_SET_FLOW_CTRL, 0,
                                          value, '', self.usb_write_timeout):
                raise pyftdiError('Unable to set flow control')
        except usb.core.USBError, e:
            raise pyftdiError('UsbError: %s' % str(e))

    def set_dtr(self, state):
        """Set dtr line"""
        value = state and pyftdi.SIO_SET_DTR_HIGH or pyftdi.SIO_SET_DTR_LOW
        if self._ctrl_transfer_out(pyftdi.SIO_SET_MODEM_CTRL, value):
            raise pyftdiError('Unable to set DTR line')

    def set_rts(self, state):
        """Set rts line"""
        value = state and pyftdi.SIO_SET_RTS_HIGH or pyftdi.SIO_SET_RTS_LOW
        if self._ctrl_transfer_out(pyftdi.SIO_SET_MODEM_CTRL, value):
            raise pyftdiError('Unable to set RTS line')

    def set_dtr_rts(self, dtr, rts):
        """Set dtr and rts lines"""
        value = 0
        value |= dtr and pyftdi.SIO_SET_DTR_HIGH or pyftdi.SIO_SET_DTR_LOW
        value |= state and pyftdi.SIO_SET_RTS_HIGH or pyftdi.SIO_SET_RTS_LOW
        if self._ctrl_transfer_out(pyftdi.SIO_SET_FLOW_CTRL, value):
            raise pyftdiError('Unable to set DTR/RTS lines')
    ...
    def set_event_char(self, eventch, enable):
        ...
        if enable:
            value |= 1 << 8
        if self._ctrl_transfer_out(pyftdi.SIO_SET_EVENT_CHAR, value):
            raise pyftdiError('Unable to set DTR/RTS lines')
    ...
    def set_error_char(self, errorch, enable):
        ...
        if enable:
            value |= 1 << 8
        if self._ctrl_transfer_out(pyftdi.SIO_SET_ERROR_CHAR, value):
            raise pyftdiError('Unable to set DTR/RTS lines')
    ...
    def set_line_property(self, bits, stopbits, parity, break_=0):
        ...
        value = bits & 0x0F
        try:
            value |= { pyftdi.PARITY_NONE : 0x00 << 8,
                       pyftdi.PARITY_ODD : 0x01 << 8,
                       pyftdi.PARITY_EVEN : 0x02 << 8,
                       pyftdi.PARITY_MARK : 0x03 << 8,
                       pyftdi.PARITY_SPACE : 0x04 << 8 }[parity]
            value |= { pyftdi.STOP_BIT_1 : 0x00 << 11,
                       pyftdi.STOP_BIT_15 : 0x01 << 11,
                       pyftdi.STOP_BIT_2 : 0x02 << 11 }[stopbits]
            if break_ == pyftdi.BREAK_ON:
                value |= 0x01 << 14
        except KeyError:
            raise AssertionError('Invalid line property')
        if self._ctrl_transfer_out(pyftdi.SIO_SET_DATA, value):
            raise pyftdiError('Unable to set line property')
    ...
    def write_data(self, data):
        ...
        try:
            while offset < size:
                ...
                if length <= 0:
                    raise pyftdiError("Usb bulk write error")
                offset += length
            return offset
        except usb.core.USBError, e:
            raise pyftdiError('UsbError: %s' % str(e))
    ...
    def read_data(self, size):
        ...
        if not self.max_packet_size:
            raise pyftdiError("max_packet_size is bogus")
        ...
        try:
            ...
            raise pyftdiError('UsbError: %s' % str(traceback.format_exc()))
        # never reached
        raise pyftdiError("Internal error")
    ...
    @classmethod
    def _get_devices(cls, vendor, product):
        """Find a list of previously open devices with the same vendor/product
                    ...
                    for configuration in dev:
                        ...
                        for interface in configuration:
                            ...
                            try:
                            	...
                            	#ignore the not implimented error thats thrown from the libusb0.1 backend
                            	pass                       
                    #dev.set_configuration()
                    
                cls.DEVICES[device] = [devs, 1]
            else:
    ...
    def _release_device(cls, usb_dev):
        ...
        try:
            for device in cls.DEVICES:
                devs, refcount = cls.DEVICES[device]
                for dev in devs:
                    if dev == usb_dev:
                        # found
                        if refcount > 1:
    ...
    def _reset_device(self):
        ...
        if self._ctrl_transfer_out(pyftdi.SIO_RESET, pyftdi.SIO_RESET_SIO):
            raise pyftdiError('Unable to reset FTDI device')
        # Invalidate data in the readbuffer
        self.readoffset = 0
        self.readbuffer = array.array('B')
    ...
    def _ctrl_transfer_out(self, reqtype, value, data=''):
        ...
        try:
            return self.usb_dev.ctrl_transfer(pyftdi.REQ_OUT, reqtype, value,
                                              self.index, data,
                                              self.usb_write_timeout)
        except usb.core.USBError, e:
            raise pyftdiError('UsbError: %s' % str(e))
    ...
    def _ctrl_transfer_in(self, reqtype, length):
        ...
        try:
            return self.usb_dev.ctrl_transfer(pyftdi.REQ_IN, reqtype, 0,
                                              self.index, length,
                                              self.usb_read_timeout)
        except usb.core.USBError, e:
            raise pyftdiError('UsbError: %s' % str(e))
    ...
    def _convert_baudrate(self, baudrate):
        ...
        #int i
        ref_clock = pyftdi.BAUDRATE_REF_CLOCK
        hispeed = False
        if baudrate <= 0:
        ...
        if self.type in ('ft2232h', 'ft2432h'):
            ...
            if baudrate > (pyftdi.BAUDRATE_REF_CLOCK>>1):
                ref_clock *= 4 # 12 MHz
                hispeed = True
